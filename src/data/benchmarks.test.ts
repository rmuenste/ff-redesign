...
import { benchmarks, benchmarkSuites, getBenchmarkBySlug } from "./benchmarks";
...
describe("benchmark registry", () => {
  ...
  it("keeps migrated benchmarks active and future pages planned", () => {
    expect(benchmarks.filter(b => b.status === "active").map(b => b.id)).toEqual([
      "rb3",
      "rb2",
      "fac3",
      "sedimentation",
  ...
  it("gives every benchmark a suite, and derives the catalogue facet from them", () => {
    for (const benchmark of benchmarks) {
      expect(benchmark.suite, benchmark.id).toBeTruthy();
    }
    expect(benchmarkSuites).toEqual(["Core benchmarks", "DNS validation"]);
    expect(new Set(benchmarks.map(b => b.suite)).size).toBe(benchmarkSuites.length);
  });
});
